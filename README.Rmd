...
Next, we subset the business channel for use in the initial EDA and model fitting pre-automation.

```{r subset_bus}
bus <- newsPop %>%
  filter(data_channel_is_bus == TRUE) %>%
    select(-starts_with("data_channel_is_"), -url, -timedelta)
```
