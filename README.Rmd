...
```{r transpose_weekdays_lifestyle}
lifestyleTP <- lifestyle %>%
  pivot_longer(starts_with("weekday_is"), names_to = "wd", values_to = "wdVal") %>%
    filter(wdVal == TRUE) %>%
      mutate(weekday = substr(wd, 12, nchar(wd))) %>%
        select(-wd, -wdVal) 
...
```{r transpose_weekdays_entertainment}
entertainmentTP <- entertainment %>%
  pivot_longer(starts_with("weekday_is"), names_to = "wd", values_to = "wdVal") %>%
    filter(wdVal == TRUE) %>%
      mutate(weekday = substr(wd, 12, nchar(wd))) %>%
        select(-wd, -wdVal) 
...
```{r weekday_contingency_bus}
GDAtools::wtable(busTP$weekday, w = busTP$shares)
```

```{r weekday_contingency_lifestyle}
GDAtools::wtable(lifestyleTP$weekday, w = lifestyleTP$shares)
```

```{r weekday_contingency_entertainment}
GDAtools::wtable(entertainmentTP$weekday, w = entertainmentTP$shares)
```

```{r weekday_contingency_tech}
GDAtools::wtable(techTP$weekday, w = techTP$shares)
```

```{r weekday_contingency_socmed}
GDAtools::wtable(socmedTP$weekday, w = socmedTP$shares)
```

```{r weekday_contingency_world}
GDAtools::wtable(worldTP$weekday, w = worldTP$shares)
```
